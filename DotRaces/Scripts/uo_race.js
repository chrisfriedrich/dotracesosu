var seconds = parseInt($("#RaceDuration").val());
var duration = seconds * 1000;

//var step1 = 625;

var percent0 = 0;
var step0 = ((percent0) * duration);
var diff0 = parseFloat($("#Race_Difference0").val());
...
var steps = [step0, step1, step2, step3, step4, step5, step6, step7, step8, step9, step10, step11, step12, step13, step14, step15, step16];
var percents = [percent0, percent1, percent2, percent3, percent4, percent5, percent6, percent7, percent8, percent9, percent10, percent11, percent12, percent13, percent14, percent15, percent16];
var diffs = [diff0, diff1, diff2, diff3, diff4, diff5, diff6, diff7, diff8, diff9, diff10, diff11, diff12, diff13, diff14, diff15, diff16];



function incrementUO(increment, speed) {
    //window.setInterval(increment, speed);
    width = parseInt($('#uoprogress').width()) + increment;
    $('#uoprogress').css('width', width + '%').attr('aria-valuenow', width);
    //$('#uoprogress').attr('transition-duration', speed);
}

function incrementOU(increment) {
    width = parseInt($('#ouprogress').width()) + increment;
    $('#ouprogress').css('width', width + '%').attr('aria-valuenow', width);
}

var uoprogress = $('#uoprogress');
var incrementDuration = duration / 90;
var x = 0;

var increment = function () {
    x = (x > 100) ? 0 : x + 1;
    uoprogress.css('width', (x % 100) + '%');
};
...
//        //var stepDistance = 90 / 16;
//        setTimeout(function () {
//            incrementUO(1, incrementDuration);
//        }, (incrementDuration));
...
//setTimeout(function () {
//    //var currentWidth = parseInt($('#uoprogress').width())
//    var width = percents[1] * 90;
//    var speed = duration / 16;
//    var currentWidth = width - (percents[0] * 90);
//    for (var i = currentWidth; i < width; i++)
//    {
//        incrementUO(i, speed);
//    }
//    //$('#uoprogress').css('width', width + '%').attr('aria-valuenow', width);
...
//    var speed = duration / 16;
//    var stepWidth = width - currentWidth;
//    for (var i = currentWidth; i < width; i++) {
//            incrementUO(i, speed);
//    }
//    //$('#uoprogress').css('width', width + '%').attr('aria-valuenow', width);
...
//    var speed = duration / 90;

//    var progressBar = $("#uoprogress");
//    var x = 0;
//    var step = 0;

//    var increment = function () {
//        if (x < 90) {
//            progressBar.css('width', (x % 90) + '%');
//            x++
//        }
//        else
//        {
